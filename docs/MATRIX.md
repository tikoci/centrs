...
- `explain` is `designed` (`commands/explain/README.md`): the explain →
  ...
  defects,pathresolve,verbsplit,write,symbols,menus,verbs}.ts` with frozen
  fixtures under `test/fixtures/explain/` — but there is **no `explain` command surface
  ...
  `src/explain/menus.ts` is the one **generated** file in that set — the
  RouterOS menu-vs-command table (#207), regenerated by
  `bun run explain:menus` from pinned restraml typed trees and drift-checked
  in QA. It keeps the offline analyzer offline; do not hand-edit it.
